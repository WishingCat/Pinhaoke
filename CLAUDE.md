...
- 关于悬浮卡中的赞助按钮不跳转 GitHub，而是打开站内赞助面板：面板顶部强调赞助后可微信联系或转账备注留名，以便加入赞助列表；随后依次展示微信赞助码、支付宝赞助码、开发者微信二维码和鸣谢赞助名单，图片直接引用 `/Images/` 下的 `wechat_sponsor.jpg`、`alipay_sponsor.jpg` 与 `MyWeChat.jpg`；Nginx 对 `/Images/` 设置 30 天 immutable 缓存，替换过内容的图片必须像两个赞助码一样带 `?v=N` 版本参数并在更换时递增，赞助二维码与鸣谢名单（姓名、金额及已提供的日期）仅在网页的赞助面板中展示，README 和其他页面区域不展示赞助内容；课程页与评测页的赞助名单保持一致，并由前端契约测试直接校验两页及展示范围。面板遵循与其它弹窗相同的焦点锁定、Escape、背景 `inert` 与关闭后焦点恢复要求；赞助按钮位于默认隐藏的悬浮卡内，因此恢复焦点前先给 `.tip-wrap` 加 `tip-pinned` 临时显示悬浮卡，聚焦后立即移除。
...
- 课程详情正文最上方仅显示最新补充的两行摘要、数量与“修正信息”按钮；点击后打开独立悬浮窗，说明用途、填写备注和查看全部补充，Esc 或返回键只关当前浮层；复用安全的留言和回复渲染。读取绑定详情的取消信号，配合课程 ID、节点存活与请求序号检查，避免切课或关闭后写入新课程的页面。
