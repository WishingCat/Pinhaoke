...
- 首页主标题保留纯文本 `h1#title`，使用独立手写字标与静态青绿高光、投影，三个学期均不显示课程搜索副标题。字标来自 [Yozai Medium v0.868](https://github.com/lxgw/yozai-font/releases/tag/v0.868)，经 fontTools 截取“拼好课”三个字、将字体名称改为 Pinhaoke Hand 后输出 `Images/fonts/pinhaoke-hand.woff2`（1808 字节）；原始版权与 SIL OFL 1.1 见 `Images/fonts/OFL.txt`。字体同源预加载、使用 swap 和系统楷体回退，仅应用于首页主标题；修改资源时同步更新 preload 与 @font-face 的版本参数。无需给生产环境增加字体处理依赖。
