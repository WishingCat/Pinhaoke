...
class ReceiverValidationTests(ReceiverTestCase):
    ...
    def test_accepts_mapping_payload_and_established_nullable_detail(self):
        with tempfile.TemporaryDirectory() as tmp:
            config = self.config(tmp)
            for detail in (None, {}):
                payload = self.valid_payload()
                payload["rows"][0]["详细信息"] = detail
                self.assertIs(validate_payload(UserDict(payload), config), payload["rows"])

            payload = self.valid_payload()
            del payload["rows"][0]["详细信息"]
            self.assertIs(validate_payload(payload, config), payload["rows"])
    ...
    def test_validates_every_row_term_basic_course_code_and_detail(self):
        with tempfile.TemporaryDirectory() as tmp:
            config = self.config(tmp)
            mutations = (
                ((), None),
...
class ReceiverHTTPTests(ReceiverTestCase):
    ...
    def running_server(self, config: ReceiverConfig):
        ...
        server = ThreadingHTTPServer(
            ("127.0.0.1", 0),
            make_handler(config, self.token),
        )
