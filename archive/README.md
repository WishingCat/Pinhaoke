...
| 文件 | 大小 | 来源 / 用途 | 替代物 |
|---|---|---|---|
| `courses.db` | 20 MB | 旧的统一 SQLite（公选+通识+专业+研究生），由 `build_db.py` 从 `Course data/` + `Graduated_Course_data/pku_graduate_courses.json` 构建 | `2026春季学期本科生课程.db` + `2026春季学期研究生课程.db` |
...
| `build_db.py` | — | 旧的 JSON→单库构建脚本 | `build_undergrad_db.py` + `build_graduate_db.py`（共用 `build_common.py`） |
...
```bash
cp archive/courses.db .
cp archive/build_db.py .
