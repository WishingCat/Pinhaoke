...
class Handler(BaseHTTPRequestHandler):
    server_version = "PkuUndergrad2627FallReceiver/1.0"
    ...
    def do_POST(self):
        ...
        print(f"[done] pageStats  : {json.dumps(payload.get('pageStats', {}), ensure_ascii=False)}", flush=True)
        print(f"[done] validation : {json.dumps(payload.get('validation', {}), ensure_ascii=False)}", flush=True)
