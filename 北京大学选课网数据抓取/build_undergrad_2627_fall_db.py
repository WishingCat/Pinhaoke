#!/usr/bin/env python3
"""Build 2026秋季学期本科生课程.db from the scraped PKU 26-27 fall JSON."""
import json
import sqlite3
import sys
...
CREATE TABLE basic_info (
    ...
    pnp             TEXT,
    notes           TEXT,
    UNIQUE(course_type, course_code, class_no, teacher)
);
...
def build():
    ...
    for item in data:
        ...
        if not course_type or not course_code:
            continue
        key = (course_type, course_code, class_no, teacher)
        if key in seen:
            duplicate_rows += 1
            continue
        seen.add(key)
