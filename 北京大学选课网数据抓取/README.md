...
秋季本科正式输入从 2026-09-07 起为 `课程数据/北大本科课程_26-27第1学期_20260907合并.json`。原始 7 月快照和 `20260907-黄庭逸选课网数据.json` 单独保留。复现这次合并时，先备份正式库，再执行：

```bash
python3 北京大学选课网数据抓取/merge_undergrad_2627_fall.py --report output/data-updates/20260907/merge-report.json
```

合并器按课程序号识别教学班，同类字段以新快照为准（新空值也覆盖旧值），保留旧快照独有教学班；跨已抓取类别的转类沿用原 ID，其他旧类别保留类别信息并同步同班的新课程字段。原 3032 个 ID 保留，新增 120 个 ID，输出 3152 条记录；拒绝重复键、学期混用、课程身份冲突、歧义转类和 ID 顺序不一致。新库不带旧译文；所有学期现均关闭翻译和语言切换，课程字段直接显示抓取的原始数据。

合并 JSON、数据库与可选审计报告先在各自目标目录的临时文件中完整生成并校验，再一起发布；校验、建库或发布失败时保留或恢复原文件，不留下新 JSON 配旧数据库的结果。三个输出必须互不重合，且不能与任一输入快照重合（包括软链接和硬链接别名）。发布中若恢复失败，会保留临时目录并在错误中给出恢复位置；进程崩溃或断电不具备跨文件事务保证，因此正式库操作前仍须备份。
