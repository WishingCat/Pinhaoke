...
当前仓库里的 `数据库/2026暑期本科生课程.db` 已经通过翻译流水线写入 10010 行暑期译文；`数据库/2026秋季学期本科生课程.db` 已写入 124734 行秋季本科译文；`数据库/2026秋季学期研究生课程.db` 暂无译文。以后如果重新运行本目录的建库脚本，会覆盖数据库并清空这些译文；需要先备份旧库，或重建后按 `北京大学课程数据翻译/README.md` 重新跑翻译流水线。
