...
合并器按课程序号识别教学班，同类字段以新快照为准（新空值也覆盖旧值），保留旧快照独有教学班；跨已抓取类别的转类沿用原 ID，其他旧类别保留类别信息并同步同班的新课程字段。原 3032 个 ID 保留，新增 120 个 ID，输出 3152 条记录；拒绝重复键、学期混用、课程身份冲突、歧义转类和 ID 顺序不一致。新库不带旧译文；所有学期保留界面语言切换，课程字段优先使用已有目标语言译文，缺失时显示原始中文。
