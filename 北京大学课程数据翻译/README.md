...
| `fall` | `数据库/2026秋季学期本科生课程.db` | 3152 | 0 |
...
网站现已关闭全部翻译，界面固定中文，列表和详情仅显示抓取的来源字段；旧 `lang` 参数仍兼容，但不启用译文。历史翻译逻辑的**回退原始中文**机制及离线工具保留，不自动运行补译。

2026-09-07 合并秋季本科数据后，所有学期现均关闭界面语言切换和课程翻译，API 返回原始课程字段。旧本科库的 159138 条译文保留在合并前备份及 Git 历史，不写入新库；秋季研究生已有译文保留但不用于显示。本次数据更新不执行翻译任务。
