...
利用 DeepSeek API 把课程内容批量翻译成 7 种语言，落入每个 DB 的 `translations` 表，由 `app.py` 在 `?lang=` 参数下即时切换。春季和暑期译文已完成；秋季本科已有一部分译文，秋季研究生暂未翻译。缺少译文时后端会保留原始中文，后续补跑翻译后会自动替换；脚本与设计见 `北京大学课程数据翻译/README.md`。
