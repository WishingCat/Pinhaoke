...
“树洞课程评测”是三个学期选项旁边的独立入口，不参与学期切换。该页面可按课程名检索树洞中的课程评测主帖及其中与课程评价有关的回复，并显示树洞号、发布时间和原帖链接。树洞卡片使用六组轮换的彩色整框；点击卡片可在站内查看数据快照中保存的完整主帖和全部回复，但评测列表仍只显示与课程评价有关的内容。搜索框输入后会自动检索，不显示输入联想；热度最高的 `24` 门课程只在点击右侧“热门课程”后展开。统计区显示数据范围 `2022-12-21` 至 `2026-07-13` 和评测数据量 `62716`，后者等于 `31642` 个主题加 `31074` 条相关回复。课程搜索页与评测页都适配桌面端、手机端和浅色/深色模式。
...
- 按课程名或教师名自动搜索树洞课程评测，或通过“热门课程”选择常用课程；列表只展示评测主帖和相关评价回复，全部主题在快照内合计另有 `145738` 条回复，点击卡片可查看对应树洞的完整内容；课程名、教师名及 `普生C`、`lsj` 等高置信缩写会加粗彩色高亮
